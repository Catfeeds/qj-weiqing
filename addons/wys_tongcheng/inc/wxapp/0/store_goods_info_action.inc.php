...
global $_GPC, $_W;
	...
	if($op=='add' || $op=='edit'){
		...
		$cnt_comment_all_store=$DBUtil->getMany('wys_tongcheng_store_comments','s_id=:s_id and pl_ctype=:pl_ctype',array(':s_id'=>$rpdata['s_id'],':pl_ctype'=>'main'));		
		$all_score=0;
		...
		$cnt_comment_all=$DBUtil->getMany('wys_tongcheng_store_comments','s_id=:s_id and pl_ctype=:pl_ctype',array(':s_id'=>$rpdata['s_id'],':pl_ctype'=>'main'),'crtime desc');
		
		$cnt_comment_all_five=$DBUtil->getMany('wys_tongcheng_store_comments','goods_id=:goods_id and pl_ctype=:pl_ctype',array(':goods_id'=>$rpdata['id'],':pl_ctype'=>'main'),'crtime desc',1,5);	
		...
		if($cnt_comment_all){
		$rpdata['cnt_comment_all']=$cnt_comment_all_five;
		$store_info['score_all']=round(floatval($all_score/count($cnt_comment_all_store )), 2);
		...
		$store_info['score_all']=0;//floatval($all_score/count($cnt_comment_all ));
		$store_info['score_m']=$this->getScore_m(5);
		$store_info['score_p']='';//$this->getScore_p(floatval($all_score/count($cnt_comment_all )));
		$rpdata['store']=$store_info;
		}
