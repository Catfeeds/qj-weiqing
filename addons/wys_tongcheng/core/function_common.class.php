...
function api_transfers($openid,$user_name,$money_sj,$tx_oncode){
        global $_GPC, $_W;
        $uniacid = $_W['uniacid'];
        $DBUtil = new DBUtil();
        ...
        $openid=$openid;//'ohzMZ0XzIIhTEtQaLsuQAdo2I1QM';//$_GPC['openid'];
        $re_user_name=$user_name;//'徐国才';//$_GPC['name'];//用户姓名
       ...
       // $mch_billno =$new_mch_billno.date('ym').substr(time(),4).substr(microtime(),2,6).rand(18,99);
        ...
        if($tx_oncode!=''){
            $tx_det=$DBUtil->getOne('wys_tongcheng_user_account_tx','oncode=:oncode',array(':oncode'=>$tx_oncode));
            $data_up=array(
            'time_tx'=>time(),
            'status_tx'=>$packpage['isok']==true?'1':'0',
            'rmk_tx'=>$packpage['error_info']
            );
            if($packpage['isok']==false){
            $data_up['enable']=-1;
            }else if($packpage['isok']=='true'){
            $data_up['enable']=1;
