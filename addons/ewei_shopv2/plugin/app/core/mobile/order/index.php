...
class Index_EweiShopV2Page extends AppMobilePage
{
	...
	public function detail()
	{
		...
		$gn = 0;
		$nog = 0;
		$goods = pdo_fetchall('select g.id,og.goodsid,og.price,g.title,g.thumb,g.status,og.total,g.credit,og.optionid,og.optionname as optiontitle,g.isverify,g.isfullback,g.storeids' . $diyformfields . '  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_goods') . ' g on g.id=og.goodsid ' . ' where ' . $scondition . ' and og.uniacid=:uniacid ', $param);
		$goods = set_medias($goods, array('thumb'));
		...
		if (!(empty($result['phone']))||$result['phone']==0) {
