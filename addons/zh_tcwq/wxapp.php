...
class Zh_tcwqModuleWxapp extends WeModuleWxapp {
    ...
    public function doPagePosting() {
        ...
        if ($res) {
            ...
            $system = pdo_get('zhtc_system', array('uniacid' => $_W['uniacid']));
            if ($system['is_jf'] == 1) {
                $res = pdo_update('zhtc_user', array('total_score +=' => $system['integral']), array('id' => $_GPC['user_id']));
                if ($res) {
  ...
  car_id=:car_id";
    ...
    public function doPageOrderInfo() {
        ...
        function getCoade($storeid) {
            ...
            function set_msg($storeid) {
                $access_token = getaccess_token();
                $data2 = array("scene" => $storeid, "page" => "zh_tcwq/pages/logs/hxorder", "width" => 400);
                $data2 = json_encode($data2);
                $url = "https://api.weixin.qq.com/wxa/getwxacodeunlimit?access_token=" . $access_token . "";
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, $url);
    ...
    public function doPageMyActivity() {
        global $_W, $_GPC;
        $where = " where a.user_id=" . $_GPC['user_id'] . " and a.state!=1";
        $pageindex = max(1, intval($_GPC['page']));
        $pagesize = $_GPC['pagesize'];
    ...
    public function doPageActCode() {
        ...
        function getCoade($storeid) {
            ...
            function set_msg($storeid) {
                $access_token = getaccess_token();
                $data2 = array("scene" => $storeid, "page" => "zh_tcwq/pages/wdbm/hx", "width" => 400);
                $data2 = json_encode($data2);
                $url = "https://api.weixin.qq.com/wxa/getwxacodeunlimit?access_token=" . $access_token . "";
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, $url);
    ...
    public function doPageHdPoster() {
        ...
        function getCoade($id) {
            ...
            function set_msg($id) {
                $access_token = getaccess_token();
                $data2 = array("scene" => $id, "page" => "zh_tcwq/pages/infodetial/infodetial", "width" => 100);
                $data2 = json_encode($data2);
                $url = "https://api.weixin.qq.com/wxa/getwxacodeunlimit?access_token=" . $access_token . "";
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, $url);
