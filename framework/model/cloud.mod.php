...
define('CLOUD_GATEWAY_URL', 'http://update.we10d.cn/api/api.php');
define('CLOUD_GATEWAY_URL_NORMAL', 'http://update.we10d.cn/api/api.php');
...
function cloud_prepare() {
    ...
    if (empty($_W['setting']['site']['key']) || empty($_W['setting']['site']['token'])) {
        return error('-1', "需要激活在线更新的朋友联系qq961999507（30元 1年）有什么问题搞不了，也可以加，不免费");
    }
    return true;
}
function cloud_build() {
    ...
    $dat = cloud_request('http://update.we10d.cn/api/api.php', $pars);
...
function cloud_download($path, $type = '') {
    ...
    $dat = cloud_request('http://update.we10d.cn/api/api.php', $pars, $headers, 300);
    if (is_error($dat)) {
...
function cloud_sms_send($mobile, $content, $postdata = array()) {
    ...
    $response = cloud_request('http://update.we10d.cn/api/api.php', $pars);
...
function cloud_cron_create($cron) {
    ...
    $result = cloud_request('http://update.we10d.cn/api/api.php', $pars);
...
function cloud_auth_url($forward, $data = array()) {
    ...
    $auth_url = 'http://update.we10d.cn/api/auth.php?__auth=' . $query;
    return $auth_url;
}
...
function cloud_flow_master_post($flow_master) {
    ...
    $dat = cloud_request('http://update.we10d.cn/api/api.php', $pars, array() , 300);
    if (is_error($dat)) {
        return error(-1, '网络存在错误， 请稍后重试。' . $dat['message']);
    }
